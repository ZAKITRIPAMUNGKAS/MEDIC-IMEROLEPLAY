<div class="grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-3 md:gap-4" style="min-height: 500px; height: auto;">
        ...
        <div class="flex-1 overflow-y-auto p-2">
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $feedbackList; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $feedback): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <button type="button" 
                         ...
                         class="w-full text-left p-2.5 md:p-3 mb-2 rounded-lg cursor-pointer transition-all border relative overflow-hidden outline-none focus:ring-2 focus:ring-sky-400
                ...
                : 'bg-white border-slate-200 hover:bg-slate-50 hover:border-slate-300'); ?>">
                        ...
                        <div class="flex items-start justify-between mb-1.5 md:mb-2">
                            <div class="flex-1 min-w-0">
                                <h4 class="font-semibold text-slate-800 text-xs md:text-sm truncate"><?php echo e($feedback->subject); ?></h4>
                        ...
                        <div class="flex items-center gap-1.5 md:gap-2 flex-wrap">
                            <!-- Type Badge -->
                            <span class="px-1.5 md:px-2 py-0.5 rounded-full text-[9px] md:text-[10px] font-semibold
                ...
                : 'bg-green-100 text-green-700'); ?>">
                            ...
                            <!-- Date -->
                            <span class="text-[9px] md:text-[10px] text-slate-500 ml-auto whitespace-nowrap">
    ...
    <div
        class="bg-white/95 backdrop-blur-sm rounded-xl lg:rounded-2xl shadow-xl border border-sky-200/50 overflow-hidden flex flex-col"
        style="min-height: 400px; max-height: 500px; height: 500px;">
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($selectedFeedback): ?>
            ...
            <div class="p-3 md:p-4 border-b border-slate-200 bg-gradient-to-r from-sky-50 to-cyan-50 shrink-0">
                <div class="flex items-start justify-between gap-2 md:gap-4">
                    <div class="flex-1 min-w-0">
                        <h3 class="font-bold text-slate-800 text-base md:text-lg mb-1 break-words"><?php echo e($selectedFeedback->subject); ?></h3>
                        <div class="flex items-center gap-2 md:gap-3 flex-wrap">
                            <span class="text-[11px] md:text-xs text-slate-600 flex items-center gap-1">
                                <i class="fas fa-user text-[10px] md:text-xs"></i>
                    ...
                    <div class="flex items-center gap-2 shrink-0">
                        <!-- Type Badge -->
                        <span class="px-2 md:px-3 py-1 rounded-full text-[10px] md:text-xs font-semibold
            ...
            <div class="flex-1 overflow-y-auto p-3 md:p-4 space-y-3 md:space-y-4">
                ...
                <div class="bg-slate-50 rounded-lg p-3 md:p-4 border border-slate-200">
                    <h4 class="font-semibold text-slate-700 text-xs md:text-sm mb-2 flex items-center gap-2">
                        <i class="fas fa-message text-sky-600 text-[10px] md:text-xs"></i>
                        Pesan
                    </h4>
                    <div class="text-slate-800 text-sm md:text-base leading-6 md:leading-7 font-medium tracking-wide prose prose-sm max-w-none break-words">
                ...
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($selectedFeedback->image): ?>
                    <div class="bg-slate-50 rounded-lg p-3 md:p-4 border border-slate-200">
                        <h4 class="font-semibold text-slate-700 text-xs md:text-sm mb-2 flex items-center gap-2">
                            <i class="fas fa-image text-sky-600 text-[10px] md:text-xs"></i>
                            Lampiran
                        </h4>
                        <div class="relative group">
                            <img src="<?php echo e(asset('storage/' . $selectedFeedback->image)); ?>" 
                                 alt="Lampiran Feedback" 
                                 class="rounded-lg max-w-full max-h-48 md:max-h-64 object-cover border border-slate-200 cursor-pointer hover:opacity-90 transition-opacity"
                ...
                <!-- Status Info -->
                <div class="bg-slate-50 rounded-lg p-3 md:p-4 border border-slate-200">
                    <h4 class="font-semibold text-slate-700 text-xs md:text-sm mb-2 md:mb-3 flex items-center gap-2">
                        <i class="fas fa-info-circle text-sky-600 text-[10px] md:text-xs"></i>
                        Status
                    </h4>

                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 md:gap-3">
                        <div>
                            <span class="text-[10px] md:text-xs text-slate-500 block mb-1">Status Saat Ini</span>
                            <span
                ...
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($selectedFeedback->notes || $selectedFeedback->status !== 'new'): ?>
                    <div class="bg-amber-50 rounded-lg p-3 md:p-4 border border-amber-200">
                        <h4 class="font-semibold text-black text-xs md:text-sm mb-2 flex items-center gap-2">
                            <i class="fas fa-sticky-note text-amber-600 text-[10px] md:text-xs"></i>
                            Catatan Admin
                        </h4>
            ...
            <!-- Actions -->
            <div class="p-3 md:p-4 border-t border-slate-200 bg-slate-50 shrink-0">
                <div class="flex items-center gap-1.5 md:gap-2 flex-wrap">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($selectedFeedback->status === 'new'): ?>
                        <button wire:click="markAsReviewed"
                            class="px-3 md:px-4 py-1.5 md:py-2 bg-yellow-500 hover:bg-yellow-600 text-white text-xs md:text-sm font-semibold rounded-lg transition-all flex items-center gap-1.5 md:gap-2">
                    ...
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($selectedFeedback->status !== 'resolved'): ?>
                        <button wire:click="markAsResolved"
                            class="px-3 md:px-4 py-1.5 md:py-2 bg-green-500 hover:bg-green-600 text-white text-xs md:text-sm font-semibold rounded-lg transition-all flex items-center gap-1.5 md:gap-2">
                            <i class="fas fa-check-circle text-[10px] md:text-xs"></i>
                            Tandai Selesai
                        </button>
                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                    ...
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($selectedFeedback->status !== 'new'): ?>
                        <button wire:click="markAsNew"
                            class="px-3 md:px-4 py-1.5 md:py-2 bg-blue-500 hover:bg-blue-600 text-white text-xs md:text-sm font-semibold rounded-lg transition-all flex items-center gap-1.5 md:gap-2">
                    ...
                    <button wire:click="deleteFeedback(<?php echo e($selectedFeedback->id); ?>)"
                        ...
                        class="px-3 md:px-4 py-1.5 md:py-2 bg-red-500 hover:bg-red-600 text-white text-xs md:text-sm font-semibold rounded-lg transition-all flex items-center gap-1.5 md:gap-2 ml-auto">
        ...
        <?php else: ?>
            <!-- Empty State -->
            <div class="flex-1 flex flex-col items-center justify-center text-center p-4 md:p-6 lg:p-8">
                <div
                    class="w-20 h-20 md:w-24 md:h-24 bg-gradient-to-br from-sky-100 to-cyan-100 rounded-xl md:rounded-2xl flex items-center justify-center mb-3 md:mb-4">
                    <i class="fas fa-comment-dots text-3xl md:text-4xl text-sky-600"></i>
                </div>
                <h3 class="font-bold text-slate-700 text-base md:text-lg mb-2">Pilih Feedback</h3>
                <p class="text-slate-500 text-xs md:text-sm max-w-sm px-4">
