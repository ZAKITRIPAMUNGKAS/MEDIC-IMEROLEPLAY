...
<div class="relative min-h-screen py-8 px-4 sm:px-6 lg:px-8">
    ...
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if((optional(auth()->user()->role)->level ?? 0) < 2 && !auth()->user()->isAdmin()): ?>
        <div class="relative max-w-4xl w-full mx-auto mt-20 text-center">
            <div class="backdrop-blur-xl border-2 border-red-400/60 rounded-2xl shadow-2xl p-8 md:p-12 bg-red-900/40">
                ...
                <h1 class="text-3xl md:text-4xl font-bold text-white mb-4">Akses Ditolak</h1>
                <p class="text-red-200 text-lg md:text-xl mb-8">
                    Maaf, Anda tidak memiliki akses untuk melihat halaman ini.<br>
                    Halaman ini hanya dapat diakses oleh Staff Co-ass (Level 2) ke atas.
                </p>
                <a href="<?php echo e(route('staff.dashboard')); ?>" class="inline-flex items-center px-6 py-3 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl transition-all duration-300 transform hover:scale-105 shadow-lg">
                ...
                <form method="GET" action="<?php echo e(route('staff.forms')); ?>" class="grid grid-cols-1 md:grid-cols-5 gap-4">
                    ...
                    <div>
                        <select name="type" class="w-full bg-white/30 backdrop-blur-xl text-white border-2 border-sky-400/40 rounded-xl px-4 py-3 focus:ring-2 focus:ring-sky-500 focus:border-sky-500 text-sm font-medium shadow-xl transition-all duration-200">
                            ...
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(auth()->user()->isAdmin() || (optional(auth()->user()->role)->level ?? 0) >= 3): ?>
                                <option value="janji_temu" class="bg-slate-800 text-slate-300" <?php echo e(request('type') == 'janji_temu' ? 'selected' : ''); ?>>Janji Temu</option>
                            <?php endif; ?>
                            
                            <?php if(auth()->user()->isAdmin() || (optional(auth()->user()->role)->level ?? 0) >= 2): ?>
                            ...
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $forms; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $form): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <tr class="hover:bg-sky-700/40 transition-colors border-b border-sky-400/30">
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        <?php elseif($form->status === 'cancelled'): ?>
                                            <div class="inline-flex items-center px-3 py-1 bg-slate-500/20 text-slate-300 rounded-full text-sm font-medium border border-slate-500/30">
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        <div class="flex flex-col sm:flex-row gap-2">
                                            ...
                                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($form->status === 'pending'): ?>
                                                ...
                                                <form method="POST" action="<?php echo e(route('staff.forms.cancel', $form->id)); ?>" onsubmit="return confirm('Yakin ingin membatalkan formulir ini? (Sesi/Antrian akan hangus)');" class="inline mt-2 sm:mt-0">
                                                    <?php echo csrf_field(); ?>
                                                    <button class="w-full inline-flex items-center justify-center px-3 py-2 bg-gradient-to-r from-slate-500 to-gray-500 text-white text-xs sm:text-sm font-bold rounded-xl hover:from-slate-600 hover:to-gray-600 transition-all duration-300 transform hover:scale-105 shadow-lg">
