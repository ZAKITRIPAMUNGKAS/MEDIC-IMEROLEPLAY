            ...
            <div class="p-4 border-b border-sky-400/30 bg-black/10 shrink-0">
                ...
                <div class="flex p-1 bg-black/20 rounded-xl">
                    <button wire:click="$set('filterStatus', 'open')"
                        class="flex-1 px-3 py-1.5 text-xs font-bold rounded-lg transition-all <?php echo e($filterStatus === 'open' ? 'bg-gradient-to-r from-sky-500 to-cyan-500 text-white shadow-lg' : 'text-sky-200 hover:text-white hover:bg-white/5'); ?>">
                        Aktif
                    </button>
                    <button wire:click="$set('filterStatus', 'closed')"
                        class="flex-1 px-3 py-1.5 text-xs font-bold rounded-lg transition-all <?php echo e($filterStatus === 'closed' ? 'bg-gradient-to-r from-sky-500 to-cyan-500 text-white shadow-lg' : 'text-sky-200 hover:text-white hover:bg-white/5'); ?>">
                        Selesai
                    </button>
                    <button wire:click="$set('filterStatus', 'all')"
                        class="flex-1 px-3 py-1.5 text-xs font-bold rounded-lg transition-all <?php echo e($filterStatus === 'all' ? 'bg-gradient-to-r from-sky-500 to-cyan-500 text-white shadow-lg' : 'text-sky-200 hover:text-white hover:bg-white/5'); ?>">
                        Semua
                    </button>
                </div>
            </div>
                    ...
                    <div class="divide-y divide-sky-400/20">
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $sessions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $session): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div wire:click="selectSession(<?php echo e($session->id); ?>)"
                                class="p-4 cursor-pointer transition-all hover:bg-white/10 group <?php echo e($activeSessionId === $session->id ? 'bg-white/10 border-l-4 border-sky-400' : 'border-l-4 border-transparent'); ?>">
                                <div class="flex justify-between items-start mb-1">
                                    <h3
                                        class="font-bold text-white group-hover:text-sky-200 transition-colors <?php echo e($activeSessionId === $session->id ? 'text-sky-300' : ''); ?>">
                                ...
                                <div class="flex justify-between items-center text-[10px] text-sky-300/60 font-medium">
                                    <span><?php echo e($session->updated_at->diffForHumans()); ?></span>
                                    <span
                                        class="px-2 py-0.5 rounded-full border <?php echo e($session->status === 'open' ? 'bg-green-500/20 text-green-300 border-green-500/30' : 'bg-slate-500/30 text-slate-300 border-slate-500/30'); ?>">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div class="min-w-0">
                            ...
                            <div class="flex items-center gap-2 text-xs text-sky-200">
                                ...
                                Dimulai <?php echo e($activeSession->created_at->format('d M H:i')); ?>
                    ...
                    <div class="space-y-4 sm:space-y-6 relative z-10 pb-4">
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $chatMessages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $msg): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <div class="flex <?php echo e($msg->is_staff_reply ? 'justify-end' : 'justify-start'); ?> group">
                                        <div
                                            class="flex flex-col <?php echo e($msg->is_staff_reply ? 'items-end' : 'items-start'); ?> max-w-[85%] sm:max-w-[75%]">
                        ...
                        <form wire:submit.prevent="sendReply" class="flex gap-3 sm:gap-4 items-end">
                            ...
                            <div class="flex-1 relative">
                                <textarea wire:model="replyMessage" rows="1" id="chat-textarea"
                                    ...
                                    placeholder="Ketik balasan untuk <?php echo e($activeSession->anonymous_name); ?>..."></textarea>
                            </div>
