...
@section('title', 'Tambah Jadwal Dokter - Panel Admin')
    ...
    <div class="relative max-w-3xl w-full mx-auto text-white">
        ...
        <div class="flex items-center justify-between mb-8">
            <div class="flex items-center">
                ...
                <div>
                    <h1 class="text-2xl sm:text-3xl font-bold text-white">Tambah Jadwal Baru</h1>
                    <p class="text-sky-300 text-sm sm:text-base">Masukkan informasi jadwal dokter secara detail</p>
                </div>
            </div>
        </div>
        ...
        <div class="glass-effect rounded-2xl elegant-shadow-lg p-6 sm:p-8">
            <form method="POST" action="{{ route('admin.doctor-schedules.store') }}" class="space-y-6">
                @csrf
                    ...
                    <select name="doctor_name" id="doctor_name" required
                        ...
                        @foreach($doctors as $doctor)
                            <option value="{{ $doctor->name }}" 
                                    ...
                                    @selected(old('doctor_name') == $doctor->name) 
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        @foreach($days as $day)
                            <label class="flex items-center p-3 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 cursor-pointer transition-all">
                                <input type="checkbox" name="day[]" value="{{ $day }}" @checked(is_array(old('day')) && in_array($day, old('day')))
                    ...
                    <div>
                        ...
                        <div class="flex gap-4">
                            <label class="flex-1 cursor-pointer">
                                <input type="radio" name="hospital" value="alta" @checked(old('hospital', 'alta') == 'alta') class="sr-only peer">
                            ...
                            <label class="flex-1 cursor-pointer">
                                <input type="radio" name="hospital" value="roxwood" @checked(old('hospital') == 'roxwood') class="sr-only peer">
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="3"
                              class="w-full bg-white/10 text-white placeholder-gray-400 border border-white/20 rounded-lg px-4 py-3 focus:ring-2 focus:ring-sky-400 focus:border-sky-400 text-sm transition-all"
                              placeholder="Keterangan tambahan atau persyaratan khusus...">{{ old('notes') }}</textarea>
                </div>
                ...
                <div class="flex flex-col sm:flex-row gap-4 pt-6 border-t border-white/10">
                    <button type="submit" class="flex-1 inline-flex items-center justify-center px-8 py-4 bg-gradient-to-r from-sky-500 to-cyan-500 hover:from-sky-600 hover:to-cyan-600 text-white rounded-xl font-bold transition-all duration-300 transform hover:scale-[1.02] shadow-xl">
                        <i class="fas fa-save mr-2"></i><span>Simpan Jadwal</span>
                    </button>
