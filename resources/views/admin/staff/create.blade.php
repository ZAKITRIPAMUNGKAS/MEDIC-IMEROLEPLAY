...
@section('title', 'Tambah Staf - Portal Medis MPK-BA')
...
<div class="relative min-h-screen py-8 px-4 sm:px-6 lg:px-8">
    ...
    <div class="relative max-w-4xl w-full mx-auto text-white">
        ...
        <div class="glass-effect rounded-2xl elegant-shadow-lg p-6 md:p-8">
            <form method="POST" action="{{ route('admin.staff.store') }}" enctype="multipart/form-data" class="space-y-8">
                @csrf
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" placeholder="Masukkan nama lengkap" class="w-full bg-white/10 text-white placeholder-gray-400 border border-white/20 rounded-lg px-4 py-3 focus:ring-2 focus:ring-sky-400 focus:border-sky-400 transition-all duration-300" required>
                            @error('name') 
                        ...
                        <div>
                            ...
                            <input type="email" name="email" value="{{ old('email') }}" placeholder="contoh@email.com" class="w-full bg-white/10 text-white placeholder-gray-400 border border-white/20 rounded-lg px-4 py-3 focus:ring-2 focus:ring-sky-400 focus:border-sky-400 transition-all duration-300" required>
                            @error('email') 
                                <p class="text-red-300 text-sm mt-2 flex items-center">
                                    <i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}
                                </p> 
                            ...
                            <div class="relative">
                                <select name="role_id" id="role_id" class="w-full bg-white/10 text-white border border-white/20 rounded-xl px-4 py-3 focus:ring-2 focus:ring-sky-400 focus:border-sky-400 transition-all duration-300 appearance-none pr-12" required>
                                    ...
                                    @foreach($roles as $role)
                                        <option value="{{ $role->id }}" data-display="{{ $role->display_name ?? $role->name }}" data-description="{{ $role->description ?? '' }}" data-permissions='@json($role->permissions ?? [])' @selected(old('role_id') == $role->id) class="bg-slate-800 text-slate-100">
