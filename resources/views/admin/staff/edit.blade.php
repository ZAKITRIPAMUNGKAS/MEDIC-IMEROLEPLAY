    ...
    <div class="relative max-w-4xl w-full mx-auto text-white">
        ...
        <div class="glass-effect rounded-2xl elegant-shadow-lg p-6 md:p-8 mb-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <a href="{{ route('admin.staff.index') }}" class="inline-flex items-center px-4 py-2 bg-white/10 text-white rounded-lg hover:bg-white/20 transition-all duration-300 text-sm font-medium backdrop-blur-sm border border-white/20">
            ...
            <div class="bg-white/5 rounded-xl p-4 border border-white/10">
                <div class="flex items-center">
                    ...
                    <div>
                        ...
                        <p class="text-gray-400 text-xs">Terakhir diperbarui: {{ optional($user->updated_at)->format('d M Y, H:i') ?? '-' }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="glass-effect rounded-2xl elegant-shadow-lg p-6 md:p-8">
            <form method="POST" action="{{ route('admin.staff.update', $user) }}" enctype="multipart/form-data" class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name', $user->name) }}" placeholder="Masukkan nama lengkap" class="w-full bg-white/10 text-white placeholder-gray-400 border border-white/20 rounded-lg px-4 py-3 focus:ring-2 focus:ring-sky-400 focus:border-sky-400 transition-all duration-300" required>
                            @error('name') 
                        ...
                        <div>
                            ...
                            <input type="email" name="email" value="{{ old('email', $user->email) }}" placeholder="contoh@email.com" class="w-full bg-white/10 text-white placeholder-gray-400 border border-white/20 rounded-lg px-4 py-3 focus:ring-2 focus:ring-sky-400 focus:border-sky-400 transition-all duration-300" required>
                            @error('email') 
                                <p class="text-red-300 text-sm mt-2 flex items-center">
                                    <i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}
                                </p> 
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="relative">
                                <input type="password" name="password" id="password" placeholder="Minimal 8 karakter" class="w-full bg-white/10 text-white placeholder-gray-400 border border-white/20 rounded-lg px-4 py-3 pr-10 focus:ring-2 focus:ring-sky-400 focus:border-sky-400 transition-all duration-300">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="password" name="password_confirmation" id="password_confirmation" placeholder="Ulangi password baru" class="w-full bg-white/10 text-white placeholder-gray-400 border border-white/20 rounded-lg px-4 py-3 pr-10 focus:ring-2 focus:ring-sky-400 focus:border-sky-400 transition-all duration-300">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="flex items-center space-x-4">
                                <img src="{{ $user->profile_image_url }}" alt="{{ $user->name }}" onerror="this.onerror=null;this.src='{{ asset('profile.jpg') }}';" class="w-20 h-20 rounded-full border-4 border-sky-400 object-cover">
                                <div>
                                    <p class="text-white text-sm font-medium">{{ $user->name }}</p>
                                    <p class="text-gray-300 text-xs">{{ $user->role->display_name ?? $user->role->name ?? 'Staff' }}</p>
                                </div>
                            </div>
                        </div>
                        
                        <div>
                            <label class="block text-sm font-medium text-gray-300 mb-3">Upload Foto Baru (Opsional)</label>
                            <div class="relative">
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select name="role_id" class="w-full bg-white/10 text-white border border-white/20 rounded-lg px-4 py-3 focus:ring-2 focus:ring-sky-400 focus:border-sky-400 transition-all duration-300 appearance-none" required>
                                @foreach($roles as $role)
                                    <option value="{{ $role->id }}" @selected(old('role_id', $user->role_id) == $role->id) class="bg-slate-800 text-slate-100">
                        ...
                        <div>
                            ...
                            <div class="flex items-center space-x-6">
                                <label class="flex items-center cursor-pointer">
                                    <input id="is_active" type="checkbox" name="is_active" value="1" class="sr-only" @checked(old('is_active', $user->is_active))>
                                    <div class="relative">
                                        <div class="w-12 h-6 {{ old('is_active', $user->is_active) ? 'bg-green-500' : 'bg-gray-500' }} rounded-full shadow-inner"></div>
                                        <div class="absolute top-1 {{ old('is_active', $user->is_active) ? 'left-7' : 'left-1' }} w-4 h-4 bg-white rounded-full shadow transform transition-transform duration-200 ease-in-out"></div>
                                    </div>
                ...
                <div class="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-4 pt-6 border-t border-white/10">
                    <a href="{{ route('admin.staff.index') }}" class="inline-flex items-center justify-center px-6 py-3 bg-white/10 text-white rounded-lg border border-white/20 hover:bg-white/20 transition-all duration-300 font-medium">
