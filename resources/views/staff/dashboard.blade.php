            ...
            @if(!(isset($anyActiveSession) && $anyActiveSession && !isset($activeSession)))
                <div class="card backdrop-blur-xl border-2 border-sky-400/60 rounded-3xl shadow-2xl animate-fade-in-up mb-8"
                    ...
                    <div class="p-6 sm:p-8">
                        ...
                        @if((isset($activeSession) && $activeSession) || $isFiveMActive)
                            <!-- Active Session -->
                            <div
                                class="bg-gradient-to-br {{ $isFiveMActive && !isset($activeSession) ? 'from-green-500/50 to-emerald-500/50 border-green-400/60' : 'from-yellow-500/50 to-orange-500/50 border-yellow-400/60' }} rounded-2xl p-4 sm:p-6 border-2 shadow-xl">
                                <div class="flex flex-col lg:flex-row items-center justify-between gap-4">
                                    <div class="flex items-center space-x-3 sm:space-x-4">
                                        <div
                                            class="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-br {{ $isFiveMActive && !isset($activeSession) ? 'from-green-500 to-emerald-500' : 'from-yellow-500 to-orange-500' }} rounded-2xl flex items-center justify-center shadow-xl animate-pulse flex-shrink-0">
                                        ...
                                        <div>
                                            ...
                                            <p
                                                class="{{ $isFiveMActive && !isset($activeSession) ? 'text-green-100' : 'text-yellow-200' }}">
                                                Dimulai
                                                {{ isset($activeSession) && $activeSession ? $activeSession->clock_in->diffForHumans() : ($fiveMClockInTime ? $fiveMClockInTime->diffForHumans() : 'Baru saja') }}
                                            </p>
